...
BSMPath::BSMPath(const BSMConfig& config): d_config(config), d_paths() {}
...
std::vector<std::vector<double>> BSMPath::generateStockPaths()
{
    ...
    std::normal_distribution<double> dis_standard_normal(0.0,
                                                         sqrt(d_config.d_time));
    for (int i = 0; i < d_config.samplesize; i++) {
        std::vector<double> path = {};
        path.push_back(
            drift * exp(dis_standard_normal(mt_rand) * d_config.d_volatility));
